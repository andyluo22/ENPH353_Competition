...
class Controller:
    ...
    def image_callback(self, msg):
        ...
        if self.terrain_timer < 1.00:
            linear_vel = 0.20

        elif self.terrain_timer >= 1.00 and self.terrain_timer < 2.90:
            linear_vel = 0.08
            angular_vel = angular_vel * 2.5

        elif self.terrain_timer >=2.90 and self.terrain_timer < 19:
            linear_vel = 0.08

            if not self.is_turning and time.time() - self.start_timer >= 1.0:
                self.is_turning = True
                self.start_turn_time = time.time()

            if self.is_turning and time.time() - self.start_turn_time <= 0.005:
                angular_vel = -0.01
            else:
                self.is_turning = False
        else:
